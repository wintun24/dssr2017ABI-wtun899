...
filename='xy_29.xlsx';%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%change file name%%%%%%%
filename1='xy_29.mat';
frameinterval=15;%%%%%%%%%change interval%%%%%%%%%%%%%
  ...
  for j = 1 : cellcount    %cell numbers
      for i=2: size(distance{j},2)
          if distance{j}(i)>= MinaveAveDis && (isnan((YY{j}(i+1)-YY{j}(1))/(XX{j}(i+1)-XX{j}(1)))~=1)
            ...
            Theta{j}(i)=atan2((YY1{j}(i+1)-YY1{j}(i)),(XX1{j}(i+1)-XX1{j}(i))); %generating angle between successive displacement for timestep (=10min)
            if Theta{j}(i)>=0 
                ThetatoVelocity{j}(i)=pi-Theta{j}(i);
            else
                ThetatoVelocity{j}(i)=-pi-Theta{j}(i);
            end        
            Velocity{j}(i)=distance{j}(i)./frameinterval; % Time step in the source code is 10min
...
final_dp_mm=final_dp.*0.37;%%%%%%%%final 


%theta with time(along or against the flow with time)
...
final_mean_speedwithT_mm=final_mean_speedwithT*0.37;
...
corrected_theta=corrected_final_theta';%%%%%%%%%%

  MaxMaxVel=max(MaxVel);
  MinMinVel=max(MinVel);
  AveVelOverCells = mean(AverageVelocity);
  AVeVelOverCells_mm=AveVelOverCells*0.37;
...
for i=1:size(dbyDPwithtime)
    ave_dbyDwithtime(i,j)=nanmean(dbyDPwithtime(i,:));%dbyD with tiem for all cells in movie
end
end
